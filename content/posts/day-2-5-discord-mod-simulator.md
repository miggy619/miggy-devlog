...
date: "2026-04-21"
summary: "Wrapped Phase 1 with a real win condition, visible wave counter, and break countdown. The game now has a start, a middle, and an end — five days in, it's officially a game."
tags: ["roblox", "luau", "devlog", "milestone", "phase-1", "game-loop"]
---
End of day 2 is the day Phase 1 ends. Two days ago this was an empty Roblox project. Today it's a playable loop: enemies attack, I ban them, I earn coins, I upgrade, I survive, I win — or I don't. 🏆
...
**Phase 1 is done in five days and it's _smaller_ than I expected it to be.**
...
What made the five days actually work:
...
Five days in, the repo has a file for every system, a devlog for every day, and a working core loop. That's a better foundation than most "hackathon" Roblox projects end up with — and it was all incremental.

## 🎯 Phase 1 — Retrospective

| Day | Feature | Outcome |
|---|---|---|
| 1 | Project + Rojo setup | Project runs |
| 2 | Enemy spawn/move, server health, Ban Hammer | Playable loop exists |
| 3 | Second enemy type, wave scaling, health UI | You can *lose* |
| 4 | Currency, shop button, first upgrade | Reason to keep playing |
| 5 | Wave counter, break countdown, win condition | ✅ Phase 1 closed |
...
Phase 2 is "game feel" — Days 6 through 12 per the plan. The code barely grows. The game transforms.

- **Days 6–7: Hit effects + enemy feedback.** Right now, banning an enemy just silently deletes the part. That needs to feel like something. Planned additions:
...
- **Day 8: More moderation tools.** Mute Gun (slows enemies), Timeout (freezes one in place), Kick (pushback cone). The Ban Hammer is one button; a real mod has a toolkit.
- **Days 9–10: Path variation + annoying enemy types.** Teleporter that skips part of the path. Splitter that becomes two Spammers on ban. Maybe a VIP troll that takes two bans.
- **Days 11–12: Real UI pass.** Proper health bar (not just text), enemy counter, round number, nicer currency display, shop panel instead of a single button.
...
Swing animation and camera shake slip to Day 7. Build the ban moment first; everything else is polish around it.
